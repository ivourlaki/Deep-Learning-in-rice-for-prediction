...
for (j in seq(11)) {
  ...
  for (i in c(1,7,8,10)) {
    ...
    if (j %in% c(1:10)) {
      marker <- read.csv(paste0("738_best_markers_snps_trait_",i,"_partitions_",j,".csv"), header=FALSE)
      marker<-marker[,-1]
      marker<-marker[-1,]
...
all.metrics <- cbind.data.frame(all.loss,all.corr,all.corr2,all.accuracy, architecture=rep("BayesC",length(all.loss[,1])),
                                marker=rep("SNPS",length(all.loss[,1])), Partition=c("Partition_1","Partition_2","Partition_3","Partition_4",
...
write.csv(all.metrics,paste0("/BAYESC_SNPS_10000_corr.csv"), row.names =TRUE)

save.image("BAYESC_SNPS_10000_corr.RData")
rm(list = ls())
