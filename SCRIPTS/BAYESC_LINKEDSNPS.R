...
for (j in seq(11)) {
  ...
  for (i in c(1,7,8,10)) {
    ...
    marker <- read.csv(paste0("LD_resulted_markers_partitions/all_LD_trait_",i,"_partitions_",j,".csv"),header=FALSE)
    marker<-marker[,-1]
    marker<-marker[-1,]
...
all.metrics <- cbind.data.frame(all.loss, architecture=rep("BayesC",length(all.loss[,1])),
...
write.csv(all.metrics,paste0("BAYESC_LINKED_SNPS.csv"), row.names =TRUE)

save.image("BAYESC_LINKED_SNPS.RData")
rm(list = ls())
