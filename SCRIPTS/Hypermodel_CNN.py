...
X_test = pd.read_csv('best_pvalues_partitions/best_markers_trait_{0}_partitions_{1}_test.csv'.format(trait,d)
                         ,header=None)
y_test = pd.read_csv('Deep_Learning/final_partitions_pheno/pheno_{0}_partitions_{1}_test.csv'.format(trait,d)
                         ,header=None)
...
class build_model(HyperModel):
    ...
    def build(self, hp):
        ...
        kernels = hp.Choice('l', values=[0.01,0.001,0.05,0.1])
        model.add(layers.Conv1D(filters=hp.Choice('num_filters',values=[16, 38, 64, 128]), 
            ...
            # second layer Polling
        model.add(layers.MaxPooling1D(pool_size=3))
        
        # Thrid layer, flatten your matrix
        ...
        #model.add(layers.Dropout(rate=hp.Choice('rate', values=[0, 0.15,0.30]))
